...
> 打响指播放/暂停、握拳静音、食指上下调节音量、手掌上下挥动切换视频，支持 B 站、YouTube 等任意网页播放器。
...
| 手势 | 动作 | 触发说明 |
| --- | --- | --- |
| 🤏 打响指（拇指 + 中指捏合） | 播放 / 暂停 | 捏合**瞬间**触发一次，防抖 900ms |
| ☝️ 食指向上 | 音量 +10% | 保持手势可连续调高（约 0.65s 一次） |
| 👇 食指向下 | 音量 -10% | 保持手势可连续调低 |
| 🖐️ 手掌向上挥动 | 下一个视频 | 张开手掌快速上挥 |
| 🖐️ 手掌向下挥动 | 上一个视频 | 张开手掌快速下挥 |
| ✊ 握拳 | 静音 / 取消静音 | 触发一次，再握一次恢复 |

> 注意：打响指不是靠“声音”识别（浏览器无法安全监听麦克风），而是靠“拇指尖与中指尖快速捏合”的动作来识别。
...
- 打响指 = 拇指与中指快速捏合（不是真的弹出响声）；
- 音量 = 食指伸直指向上/下，保持姿势可连续调节；
- 切集 = 手掌张开，快速向上/向下挥动一次。
...
- 打响指需要拇指与中指“快速捏合”，捏住不放不会重复触发（防抖设计）；
