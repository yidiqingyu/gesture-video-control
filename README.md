...
> OK 手势播放/暂停、握拳静音、小拇指上下调节音量、食指上下切换视频，支持长/短视频双模式，B 站、YouTube 等任意网页播放器。
...
- 支持**悬浮窗**：可拖动、可隐藏摄像头画面、可最小化为悬浮球，识别全程不中断
- 「开启 / 关闭手势控制」一键开关，开关状态自动记忆
- **防抖 + 稳定帧检测**：手势需连续稳定数帧才会触发，避免误触
- 音量手势支持**长按连续调节**
- 适配 YouTube / B 站「下一集 / 上一集」按钮，其它网站有通用兜底
- **长 / 短视频双模式**：手掌张开保持 2 秒一键切换；短视频模式下食指上下变为滑动刷视频，贴合手机刷视频习惯
...
| ☝️ 单个食指向上 | 长视频：上一个视频 / 短视频：向下滑动 | 保持手势触发一次 |
| 👇 单个食指向下 | 长视频：下一个视频 / 短视频：向上滑动 | 保持手势触发一次 |
| 🖐️ 手掌向上挥动 | 下一个视频 | 张开手掌快速上挥 |
| 🖐️ 手掌向下挥动 | 上一个视频 | 张开手掌快速下挥 |
| 🖐️ 手掌张开保持 2 秒 | 切换长 / 短视频模式 | 切换一次后需松手重新比一次 |
| ✊ 握拳 | 静音 / 取消静音 | 触发一次，再握一次恢复 |
...
- **短视频模式**（刷竖屏视频流，如抖音网页版、B 站推荐流、YouTube Shorts）：食指向上 = 向下滑动一屏，食指向下 = 向上滑动一屏，还原用手指刷视频的感觉；
- 切换方式：手掌张开（五指向上）保持 **2 秒**；再保持 2 秒切回。也可以直接在弹窗 / 悬浮窗里点「短视频模式」开关。
...
4. `content.js` 找到页面主 `<video>` 执行播放/暂停/音量/静音，或点击“下一集/上一集”按钮；
...
├── float.html           # 悬浮窗：可拖动 / 隐藏画面 / 最小化为悬浮球
├── float.css            # 悬浮窗样式
...
- 在弹窗中点击「🪟 打开悬浮窗」，会弹出一个可拖动的悬浮小窗；
- 悬浮窗显示摄像头画面与当前手势，点标题栏的 **👁** 可显示 / 隐藏画面，点 **—** 可最小化为一个小悬浮球（识别不中断），点 **✕** 关闭悬浮窗（识别同样不中断）；
- 悬浮球上的圆点变绿表示后台识别正在运行，点悬浮球即可恢复窗口。
...
- 切集 = 单个食指伸直朝上/朝下一次，或手掌张开快速上/下挥动一次。
